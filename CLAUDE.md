...
摩斯识别、计时器、计数器、连发器、识别触发、息屏；识别触发支持快捷键单次/按住持续、多参考图区域匹配、多区域识色，截图/NCC 通过全局 `Semaphore(2)` 调度到 `spawn_blocking`，旧 watcher generation 不得继续触发效果；前端 reducer、CardEditor、overlay 分模块维护。外加攻略网站工作台。
...
取消 ≠ 清空。`cancel_in_flight`（内部 `cancel_runtime`）bump generation + `notify_waiters` + 清节奏，但**保留** `qualifiedRuleIds` / `consumedRuleIds` / `currentSessionRuleIds` / `activeRoundTargets`，且 `ActiveRound` 相位不降级（降级会让 `rollback_failed_round_start` / `end_active_round` 匹配不到相位 -> 暂存资格永久悬空）。`invalidate`（内部 `reset_runtime`）才连当天资格一起清。默认用 `cancel_in_flight`：导航失败、人工判定、一键恢复、settings autosave、`set_paused` 双向、scheduler 暂停、资源释放、审计保存失败都只是让 in-flight 查询作废，「当天哪些规则达标」没有失效。`invalidate` 只留给身份真的没了：切 Profile、应用关闭、利润查询配置本身被改。`sync_window` 同构分流——换天 `reset_runtime(Disabled)`，仅 `settingsRevision` 前进只 `cancel_runtime`，`paused` 分支只取消并用 `!matches!(phase, Paused | ActiveRound)` 去重；`begin_query` / `begin_cutoff_query` 一致。`with_expected_revision_change` 让每次 settings 写入都 bump revision，把 revision 当身份变化 -> 任何 autosave / 人工判定 / 一键恢复都清资格 -> `profit_gate_for_round` 拿到 `Qualified(∅)` -> 利润 gate 下的子弹目标全被滤掉，达标也不提前兑换且无报错。`special_ops_save_settings` 必须用 `profit_query_identity_changed` 分流，只比较 `profitFilter.enabled` / `cutoffTime` / `rules` / `dailyExchangeTime` 四项查询输入；审计与 `cutoffState` 是查询产物，延迟、点击点、账号顺序与查询无关。达标必须**立刻**兑换：`build_round_plan_with_profit` 必须用带快照的 `build_schedule_with_profit_runtime`，`profit_gate_for_round` 的 `ProfitRuntimeSnapshot` 要经 `freeze_round_run` 透传到 planner。投影只在拿到 `qualifiedRuleIds` 时才把达标子弹提到「现在」；无快照会退到 `WaitingQuery` 排到 `cutoffAtMs` -> `is_due` 恒 false -> 空计划 -> `EMPTY_ROUND_PLAN_ERROR` 被 `is_transient_round_launch_error` 判成瞬时故障只 warn + `RetryAfter(30s)` -> poll（带快照）说该启动、freeze（无快照）算出空计划，每 30 秒对拆一次直到截止时间才兑换，全程无报错。
...
单项判定入口不能只看 `TimelineTask.manualFailure`：`NavigationTimedOut` 只写 `ManualCheckRequired` 且两个定位字段均为空，制作台 `Uncertain` 或账号 `ManualCheckRequired` 也必须出现入口，只有 `NeedsManualLogin` / `LoginFailed` 退回账号页（前端 `timelineTaskAllowsInlineCorrection` 与后端 `account_blocks_task_correction` 同步）。选“正在制作”时剩余时间预填存量计时，留空或 0 表示继承 `finishesAtMs`，后端无可继承值才拒绝。任何清账号状态的路径都必须同时还原 `Uncertain` 制作台，否则它在调度与任务栏双重过滤下永久消失。`account_allows_manual_check` 必须是 `!matches!(status, Ready)`，覆盖全部非 `Ready`（含 `Uncertain` / `Isolated`）：前端对这两种状态同样渲染「已人工检查」，gate 漏掉就只拿到一句被顶部横幅吞掉的报错 -> 表现为「任务栏说去账号页处理，账号页点了没反应，只有一键恢复能救」。账号级动作（已人工检查 / 一键恢复）的失败必须渲染在按钮旁的 inline alert，页头横幅在账号列表滚动位置上看不见。`Isolated` 由子弹目标失败升级而来，清账号状态时必须同时清 `lastFailure.ammoTargetId` 指名的那个目标的 `lastFailure`（其他目标不动），否则账号回 `Ready` 却永远少跑一种子弹。`special_ops_restore_account_state` 一键恢复单账号或全部账号，并清当天 `lastSuccessDay` -> 目标回未兑换、可再次调度；重复兑换由流程内资格与库存检查分支兜底。两个一键恢复按钮常驻显示，无可恢复项时 disabled 并在 title 说明原因；前端 `accountRestorable` 必须与后端 `changed` 判定一致（含当天 `lastSuccessDay`），跨天判定用 `shanghaiDay` 对齐 Rust 固定 UTC+8 的 `local_day_and_minute`。

### 特勤处自动暂停与任务栏顺序

自动暂停必须写 `SpecialOpsSettings.pausedReason` 并在页头展示，否则用户看到的是无原因的静默停摆。`special_ops_set_paused` 两个方向都清空该字段；`special_ops_save_settings` 强制沿用进程内 `paused` / `pausedReason`，前端草稿不得回滚运行态。scheduler 晚醒判定只用 poll 成功返回的 `nowMs`；启动到期轮次的空计划、暂停中、总开关关闭、试运行未清理、revision 陈旧经 `is_transient_round_launch_error` 分流为 `RetryAfter(30s)`，不得全局暂停。空计划文案必须用常量 `EMPTY_ROUND_PLAN_ERROR` 同时供 `freeze_round_run` 抛出与 `TRANSIENT` 列表匹配，禁止两处各写字面量：漂移后分流失配 -> 利润达标当天被全局暂停吞掉，而暂停又让 `sync_window` 走 `paused` 分支 `reset_runtime` 清空 `qualifiedRuleIds` -> 当天不再兑换且原因不可见。轮次切换关闭游戏（导航超时后、账号失败后、会话结束）失败只记 warn 并继续本轮，禁止全局暂停：登录头两步 `StopGame` / `StopWeGame` 无条件重杀两个 exe -> 残留进程下轮自愈。转场关游戏发结束信号后不等待退出（ACE 上 WaitForSingleObject 会无视超时，死等会卡死整轮）；扫描卡死 5 秒放弃。登录 StopGame / StopWeGame 只带查询权限核对路径，再聚焦主窗口 Alt+F4；前台仍属该进程树则 Enter 确认退出框，随后对进程树全部窗口发 WM_CLOSE / SC_CLOSE（WeGame 把 Alt+F4 当托盘隐藏，确认框是 owned 窗口，Alt+F4 成功不得提前返回）。OpenProcess 不得带 PROCESS_TERMINATE 做路径查询：ACE 会错误 5 或 31。查询被拒仍按文件名关窗，不得把 ACE 拒绝查询当致命失败，否则 Alt+F4 跑不到。路径只核一次，后续只看快照 PID 是否还在，禁止每 100ms 再 OpenProcess 查询：那会把 ACE 偶发错误 31 打成几乎每次切号都失败。强杀前启用 SeDebugPrivilege，失败忽略。随后按进程快照轮询最多 15 秒，快照已空或 PID 已消失即成功，预算耗尽且仍有精确匹配才失败。只有 `stop_for_pause` 的 `PauseRequested` 路径保留 `round.closeGame`，且暂停原因先落盘 -> 进程错误文本不进 `pausedReason`。交易行窗口内制作台优先：进 `market.entry` 之前若已有到期制作立即让位；循环内只让位给入口后新到期的制作（`latest_due_craft_at_ms()` 取 `max` 与基线比较）。循环内禁止回到 `next_craft_at_ms()` 式的「全部制作取 min 且 `<= now` 就让位」，否则仍排在交易行后面的队列任务会让每买一件就退出换号 -> 配置购买次数永远停在 1。让位时点击 `game.specialOps` 从交易行/大厅进四制作台；同账号保持会话，跨账号才关游戏切号。`market.price` OCR/截图失败按未识别页累计。连续三页失败把该账号交易行插到剩余到期任务队尾重试一次（插在远期 lookahead 之前）；队尾补偿仍失败则写入 `priceRetryAtMs = now + 1h` 并标记 `PriceRecognitionFailed` 供展示，planner 只在冷却结束且窗口仍开时再次到期，新一轮仍走「三页失败→队尾重试」。循环直到交易行窗口结束。禁止升级成系统暂停。一键恢复清冷却并放回 `Pending`。购买按钮识别与点击分离：识别用 `craft.purchase` / `ammo.purchase`（RecognitionRegion），点击用 `craft.purchaseClick` / `ammo.purchaseClick`（ClickPoint，由对应识别区域作 `guardAnyOf` 守卫），映射集中在 `click_target_key()`；冻结配置缺点击点时回落识别区域，preflight 负责要求补校准。所有点击路径都必须走 `click_target_key()`（含 `click_unverified`），否则同一目标经旁路点击时静默落回识别区域。制作购买点击用 `click_unverified("craft.purchase", true, _)`：三条到达该点击的路径（首次 `wait_ready`、命中 Fill 后 `wait_ready`、命中 Purchase 后 `wait_button`）都已确认过同一张模板，`click()` 内层 `verify` 会紧接着重复一次双采样，每次白付一个 `SAMPLE_INTERVAL`（400ms）却拿不到新信息。补齐/购买 3 次判定仓库不足的流程不变。`timelineTasks` 按执行顺序返回：到期任务按 `account.order` 分桶在前，未到期任务按时间在后且保留账号顺序作次键。交易行**全局**排最后：非交易行业务（制作 / 子弹 / 限时商品）按账号顺序合并成每账号一桶，交易行独立成桶并整体追加在全部非交易行桶之后（账号 1 特勤处 → 账号 2 特勤处 → 账号 1 交易行）。禁止在分桶后再按 `(account_order, scheduled_at_ms)` 整体重排，那会把交易行桶塞回它自己账号后面 -> 账号 1 交易行插到账号 2 之前。无其他账号非交易行任务时两桶相邻且同账号，`can_chain_follow_up` 保持会话 -> 不重新登录。冻结缓存键必须是 `FrozenRoundAccountKey = (String, i64, bool)`，第三位是「是否交易行桶」：两者 `scheduledAtMs` 都分钟对齐（子弹取每日兑换时间、交易行取窗口起点），撞同一分钟时二元组相同 -> `collect()` 只留后插入的交易行桶 -> 非交易行桶拿到 `craft: []` / `ammo: None`，制作与子弹被静默跳过。当前周期的限时商品任务**检查完即出栏**：`build_timeline_tasks` 只有 `Pending` 状态才进入任务栏，`NoHighValue` / `HighValue` / `Failed` 任意终态立刻出栏。出栏后 planner 的 `limited_supply_due`（同样只认换周期或 `Pending`）也匹配不到 -> 本周期不可重跑。两侧 gate 同源，禁止放宽任一侧接受终态。重跑只由 `special_ops_recheck_limited_supply` 人工触发（复位 `LimitedSupplyAccountState` 到 `Pending`，保留 `cycleId`）；两个入口均在账号人工校正面板 `CorrectionLimitedSupply`：未确认 `HighValue` 时显示"已查看高价值商品"按钮，四种终态全显示"重新检查"按钮。任务栏不再渲染 `limitedOutcome` 结果行或"已查看高价值商品"按钮。`compare_samples` 判定 `HighValue` 要求两次采样的命中区域集合**完全相等**且该区域两次命中的目标颜色相同：取交集非空就判定会把命中抖动（1,3 -> 3）算成一致，只读第二次颜色会把跨色命中算成稳定，两者都是误报高价值。采样必须是滑动窗口（比对失败时 `previous = Some(sample)` 而不是 `take()`），切成互不相交的对会让一次抖动毁掉整对。`limitedSupply.researchDelayMs` 必须真正接到 `LimitedRunConfig.enter_delay` 并在 `wait_ready` **之前**消费：点完 `ammo.researchDepartment` 立刻采样时 `limited.ready` 会在上一页连续命中放行 -> 识色跑在错误页面、两次同样「稳定」-> 800ms 内写下高价值，表现为只点到研发部门就关游戏并标记发现高价值。`market.backToEntry`（校准名「返回大厅列表点击点」）只在同账号桶真的有子弹或限时商品任务时点（`!task.ammo_target_ids.is_empty() || task.limited_supply_cycle_id.is_some()`），用于从部门页退回大厅列表；交易行流程开头无条件点会把已在大厅的界面带进别的面板。`restore_account_state` 必须把当天 `Running` / `PriceRecognitionFailed` / `WindowClosed` 的交易行放回 `Pending`（`Completed` 与其他日期不动，购买次数已花掉）：任务栏对当天已买满次数的 `Completed | WindowClosed` 直接 `continue`，而 planner 依赖该任务栏任务 -> 一次 `WindowClosed` 让交易行当天永久消失 -> 一键恢复后点「继续」不再跑交易行。交易行任务栏必须渲染 `已购买 N/M · <状态>`（`marketCompletedCount` / `marketTargetCount` / `marketStatus` 后端一直在下发），否则上调购买次数后任务栏毫无变化 -> 用户以为配置没生效。当天两个终态（`Completed` / `WindowClosed`）的出栏都必须对购买次数敏感——`completedCount >= purchaseCount` 才出栏，次数从 1 调到 3 时任务立刻回栏继续买；planner 的 `market_purchase_due` 因此不再筛状态白名单，只看 `completedCount < purchaseCount`，与任务栏严格互补（planner 更严 -> 「任务栏有任务、点继续却不执行」）。例外：当天 `PriceRecognitionFailed` 任务栏保留、planner 跳过直到一键恢复，避免 OCR 失败空转或升级成系统暂停。`WindowClosed` 只在 `minute >= windowEndMinute` 时写入而 `is_current` 蕴含窗口开着，所以当天能看见它只有「窗口结束时间被人为延后」一种情形，那正是应该继续买的场景。
