...
pub use self::types::{
    ...
    RapidfireRunStatus, RapidfireSelectionKind, RapidfireSettings,
...
// 内部引用：mod.rs 自身代码使用
use self::keys::{
    parse_target_key, RAPIDFIRE_DISPLAY_MIN_WIDTH,
    RAPIDFIRE_DISPLAY_MAX_WIDTH, RAPIDFIRE_MIN_INTERVAL_MS,
    RAPIDFIRE_PRESS_JITTER_MIN_MS, RAPIDFIRE_PRESS_JITTER_MAX_MS,
    RAPIDFIRE_GLOBAL_DELAY_MIN_MS, RAPIDFIRE_GLOBAL_DELAY_MAX_MS,
    RAPIDFIRE_TRIGGER_JITTER_MAX_MS,
};
use self::overlay::{
    display_label_for_group, destroy_position_windows,
    destroy_display_windows, hide_display_windows,
};
use self::worker::{
    next_session_id, spawn_session_worker, stop_latest_active_session,
    stop_all_sessions,
};
...
async fn handle_key_down(app: &AppHandle, card_ids: Vec<String>) -> Result<(), String> {
    ...
    let sessions_to_spawn = {
        ...
        let mut card_infos: Vec<(String, String, String, u64, u64, u64, u64, u64, bool, bool, bool)> = Vec::new();
        for card_id in &card_ids {
            let cid = card_id.clone();
            if let Some(info) = inner.settings.cards.iter()
                .find(|c| c.id == cid && c.enabled && group_enabled(&inner.settings.groups, &c.group_id))
                .map(|c| (c.id.clone(), c.trigger_key.clone(), c.target_key.clone(),
                          c.interval_ms, c.press_jitter_min_ms, c.press_jitter_max_ms,
                          c.min_press_spacing_ms, c.trigger_jitter_max_ms,
                          c.cancel_jitter_on_release, c.skip_compensation, c.ignore_trigger_key))
            {
        ...
        for (cid, trigger, target, interval, jitter_min, jitter_max,
             min_press_spacing_ms, trigger_jitter_max_ms,
             cancel_jitter_on_release, skip_compensation, _) in card_infos
        {
            ...
            sessions_to_spawn.push(RapidfireSessionWorker {
                card_id: cid, session_id, trigger_key: trigger, target_key: target,
                interval_ms: interval, press_jitter_min_ms: jitter_min,
                press_jitter_max_ms: jitter_max, skip_compensation,
                compensation_delay_min_ms, compensation_delay_max_ms,
                min_press_spacing_ms, trigger_jitter_max_ms,
                cancel_jitter_on_release, control_rx,
                compensate_now, last_press_at,
            });
        }

        if ignore_trigger_key_for_batch {
            let trigger_keys: HashSet<String> = sessions_to_spawn.iter().map(|w| w.trigger_key.clone()).collect();
    ...
    if spawned_count > 0 {
        let bootstrap = {
            let inner = state.lock_inner().map_err(|_| "连发器状态已损坏".to_string())?;
...
async fn handle_key_up(app: &AppHandle, card_ids: Vec<String>) -> Result<(), String> {
    let state = app.state::<RapidfireState>();
    let stopped_count = {
        let mut inner = state.lock_inner().map_err(|_| "连发器状态已损坏".to_string())?;
        if !inner.settings.rapidfire_enabled { return Ok(()); }

        let ignore_trigger_keys: Vec<String> = inner.settings.cards.iter()
            .filter(|c| card_ids.contains(&c.id) && c.ignore_trigger_key)
            .map(|c| c.trigger_key.clone()).collect();
        ...
        for trigger_key in &ignore_trigger_keys {
            let has_active = inner.settings.cards.iter()
                .filter(|c| c.trigger_key == *trigger_key && c.ignore_trigger_key && c.enabled)
                .any(|c| inner.logic.runs.get(&c.id).map(|run| !run.sessions.is_empty()).unwrap_or(false));
            if !has_active {
                let hm = app.state::<HotkeyManager>();
                match hm.unsuppress_key(trigger_key) {
                    Ok(was) => eprintln!("[连发器] unsuppress_key({trigger_key}) -> was_suppressed={was}"),
...
pub(crate) fn normalize_card(card: &RapidfireCard) -> Result<RapidfireCard, String> {
    ...
    if name.is_empty() { return Err("连发器卡片名称不能为空".to_string()); }

    let trigger_key = normalize_trigger_key(&card.trigger_key).map_err(|e| format!("{name} 的触发键{e}"))?;
    if trigger_key.is_empty() { return Err(format!("{} 的触发键不能为空", name)); }
    let trigger_primary = keys::trigger_primary_label(&trigger_key)?;
    if parse_target_key(&trigger_primary).is_none() { return Err(format!("{} 的触发键不支持: {}", name, trigger_primary)); }

    let target_key = normalize_single_key(&card.target_key).map_err(|e| format!("{name} 的目标键{e}"))?;
    if target_key.is_empty() { return Err(format!("{} 的目标键不能为空", name)); }
    if parse_target_key(&target_key).is_none() { return Err(format!("{} 的目标键不支持: {}", name, target_key)); }

    let interval_ms = card.interval_ms.max(RAPIDFIRE_MIN_INTERVAL_MS);
    let press_jitter_min_ms = card.press_jitter_min_ms.max(RAPIDFIRE_PRESS_JITTER_MIN_MS).min(RAPIDFIRE_PRESS_JITTER_MAX_MS);
    let press_jitter_max_ms = card.press_jitter_max_ms.max(RAPIDFIRE_PRESS_JITTER_MIN_MS).min(RAPIDFIRE_PRESS_JITTER_MAX_MS);
    if card.min_press_spacing_ms > RAPIDFIRE_GLOBAL_DELAY_MAX_MS {
        return Err(format!("{} 的按键最小间距不能大于 {}ms", name, RAPIDFIRE_GLOBAL_DELAY_MAX_MS));
    }
    if card.trigger_jitter_max_ms > RAPIDFIRE_TRIGGER_JITTER_MAX_MS {
        return Err(format!("{} 的触发抖动延迟上限不能大于 {}ms", name, RAPIDFIRE_TRIGGER_JITTER_MAX_MS));
    }
    if press_jitter_min_ms > press_jitter_max_ms { return Err(format!("{} 的目标键触发抖动最小值不能大于最大值", name)); }

    Ok(RapidfireCard {
        id: card.id.trim().to_string(), group_id: card.group_id.trim().to_string(),
        name: name.to_string(), trigger_key, target_key, interval_ms,
        press_jitter_min_ms, press_jitter_max_ms,
        ...
        enabled: card.enabled, skip_compensation: card.skip_compensation,
...
fn default_rapidfire_group(settings_value: &RapidfireSettings) -> RapidfireGroup {
    RapidfireGroup {
        id: DEFAULT_RAPIDFIRE_GROUP_ID.to_string(), name: "默认分组".to_string(),
        enabled: true, show_overlay: settings_value.show_overlay,
...
fn normalize_groups(settings_value: &RapidfireSettings) -> Result<Vec<RapidfireGroup>, String> {
    ...
    } else { settings_value.groups.clone() };
    ...
    for mut group in groups {
        ...
        if group.id.is_empty() { group.id = DEFAULT_RAPIDFIRE_GROUP_ID.to_string(); }
        group.name = group.name.trim().to_string();
        if group.name.is_empty() { return Err("连发器分组名称不能为空".to_string()); }
        if seen.insert(group.id.clone(), true).is_some() { return Err(format!("连发器分组 ID 重复: {}", group.id)); }
        ...
        group.overlay_width = group.overlay_width.max(RAPIDFIRE_DISPLAY_MIN_WIDTH).min(RAPIDFIRE_DISPLAY_MAX_WIDTH);
...
fn normalize_single_key(raw: &str) -> Result<String, String> {
    let trimmed = raw.trim();
    if trimmed.is_empty() { return Ok(String::new()); }
    if trimmed.contains('+') { return Err("必须是单键，不能包含组合键".to_string()); }
...
pub(crate) fn normalize_settings(mut settings_value: RapidfireSettings) -> Result<RapidfireSettings, String> {
    settings_value.overlay_width = settings_value.overlay_width.max(RAPIDFIRE_DISPLAY_MIN_WIDTH).min(RAPIDFIRE_DISPLAY_MAX_WIDTH);
    ...
    settings_value.min_press_spacing_ms = settings_value.min_press_spacing_ms
        .max(RAPIDFIRE_GLOBAL_DELAY_MIN_MS).min(RAPIDFIRE_GLOBAL_DELAY_MAX_MS);
    settings_value.trigger_jitter_max_ms = settings_value.trigger_jitter_max_ms
...
pub(crate) fn restart_hotkey_listeners(
    state: &RapidfireState, hotkey_manager: &HotkeyManager,
    settings_value: &RapidfireSettings, force: bool,
) -> Result<(), String> {
    if !settings_value.rapidfire_enabled { return hotkey_manager.clear_hold_scope("rapidfire"); }

    let mut new_by_key: HashMap<String, Vec<String>> = HashMap::new();
    for card in &settings_value.cards {
        if !card.enabled || !group_enabled(&settings_value.groups, &card.group_id) { continue; }
        new_by_key.entry(card.trigger_key.clone()).or_default().push(card.id.clone());
    }
    ...
    let previous_by_key = {
        ...
        for card in &inner.settings.cards {
            if !card.enabled || !group_enabled(&inner.settings.groups, &card.group_id) { continue; }
            by_key.entry(card.trigger_key.clone()).or_default().push(card.id.clone());
        }
        by_key
    };

    if !force && new_by_key == previous_by_key { return Ok(()); }
...
pub fn stop_all(app: &AppHandle, state: &RapidfireState, hotkey_manager: Option<&HotkeyManager>) {
    let bootstrap = {
        let Ok(mut inner) = state.lock_inner() else { return; };
        stop_all_sessions(&mut inner.logic.runs, SessionControl::Cancel);
        inner.logic.runs.clear();
        RapidfireLogic::build_bootstrap(&inner)
    };
...
pub(crate) fn stop_registered(app: &AppHandle) -> Result<(), String> {
    let Some(state) = app.try_state::<RapidfireState>() else { return Ok(()); };
    let hotkey_manager = app.try_state::<HotkeyManager>();
    stop_all(app, &state, hotkey_manager.as_ref().map(|v| &**v));
    Ok(())
}
...
pub fn initialize(app: &AppHandle, hotkey_manager: &HotkeyManager) -> Result<RapidfireState, String> {
    ...
    let logic = RapidfireLogic { runs: HashMap::new(), pending_position: None };
    ...
    if settings.rapidfire_enabled {
        if let Err(error) = restart_hotkey_listeners(&state, hotkey_manager, &settings, true) {
            if let Ok(mut inner) = state.lock_inner() { inner.hotkey_error = Some(error); }
        }
...
mod tests {
    ...
    fn sample_card(id: &str, trigger: &str) -> RapidfireCard {
        RapidfireCard {
            id: id.to_string(), group_id: DEFAULT_RAPIDFIRE_GROUP_ID.to_string(),
            name: id.to_string(), trigger_key: trigger.to_string(), target_key: "1".to_string(),
            interval_ms: 100, press_jitter_min_ms: RAPIDFIRE_DEFAULT_PRESS_JITTER_MIN_MS,
            press_jitter_max_ms: RAPIDFIRE_DEFAULT_PRESS_JITTER_MAX_MS,
            min_press_spacing_ms: 80, trigger_jitter_max_ms: 0,
            cancel_jitter_on_release: true, enabled: true,
            skip_compensation: false, ignore_trigger_key: false,
        }
    }

    #[test]
    fn normalize_card_rejects_empty_name() {
        let mut card = sample_card("a", "F1"); card.name = "  ".to_string();
        assert!(normalize_card(&card).unwrap_err().contains("名称不能为空"));
    }

    #[test]
    fn normalize_card_clamps_interval_to_minimum() {
        let mut card = sample_card("a", "F1"); card.interval_ms = 0;
        assert_eq!(normalize_card(&card).unwrap().interval_ms, RAPIDFIRE_MIN_INTERVAL_MS);
    }

    #[test]
    fn normalize_card_clamps_press_jitter_to_supported_range() {
        let mut card = sample_card("a", "F1"); card.press_jitter_min_ms = 0; card.press_jitter_max_ms = 2500;
    ...
    fn normalize_card_rejects_inverted_press_jitter_range() {
        let mut card = sample_card("a", "F1"); card.press_jitter_min_ms = 30; card.press_jitter_max_ms = 20;
        assert!(normalize_card(&card).unwrap_err().contains("触发抖动"));
    }

    #[test]
    fn normalize_card_preserves_skip_compensation() {
        let mut card = sample_card("a", "F1"); card.skip_compensation = true;
        assert!(normalize_card(&card).unwrap().skip_compensation);
    }

    #[test]
    fn normalize_card_rejects_unsupported_target_key() {
        let mut card = sample_card("a", "F1"); card.target_key = "UnknownKey".to_string();
    ...
    fn normalize_card_keeps_alt_as_trigger_primary_key() {
        assert_eq!(normalize_card(&sample_card("a", "alt")).unwrap().trigger_key, "Alt");
    }

    #[test]
    fn normalize_card_rejects_unsupported_trigger_key() {
        assert!(normalize_card(&sample_card("a", "F25")).unwrap_err().contains("触发键不支持"));
    }

    #[test]
    fn normalize_card_normalizes_key_labels() {
        let mut card = sample_card("a", "space"); card.target_key = "escape".to_string();
        let n = normalize_card(&card).unwrap();
        assert_eq!(n.trigger_key, "Space"); assert_eq!(n.target_key, "Esc");
    }

    #[test]
    fn normalize_settings_auto_adds_default_card_when_empty() {
        let mut s = RapidfireSettings::default(); s.cards.clear();
        assert_eq!(normalize_settings(s).unwrap().cards.len(), 1);
    }

    #[test]
    fn normalize_settings_migrates_legacy_group() {
        let mut s = RapidfireSettings::default(); s.groups.clear(); s.show_overlay = true;
        s.overlay_width = 420; s.overlay_position = Some(RapidfireRect { x: 11, y: 22 });
        s.cards = vec![sample_card("a", "F1")];
        let n = normalize_settings(s).unwrap();
        assert_eq!(n.groups.len(), 1); assert!(n.groups[0].show_overlay);
    ...
    fn normalize_settings_rejects_inverted_global_compensation_delay() {
        let mut s = RapidfireSettings::default();
        s.compensation_delay_min_ms = 180; s.compensation_delay_max_ms = 120;
