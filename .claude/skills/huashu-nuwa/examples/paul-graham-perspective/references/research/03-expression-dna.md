...
> 调研日期：2026-04-05
> 调研目标：从Twitter/X、essays、争议立场中提取PG的语言指纹，为skill建设提供表达层素材

---

## 1. 风格总论：伪装成简单的精密

PG的写作风格常被概括为"像说话一样写作"，但多位分析者指出这是一种精心构造的幻觉。
...
- 企业黑话（synergy, leverage作动词, paradigm shift等）
...
PG的essays经常用自问自答推进论证，类似莎士比亚独白的结构。他不是在一句话里压缩问题，而是展示思考的每一个阶段。
...
- 连续提问
- 高低文体混搭（highbrow idea + colloquial expression）
...
- 偏好1-3句的短推文
- 频繁使用对比结构（A, but B / not X, but Y）
- 很少使用emoji或hashtag
- 经常附带链接分享（并批评X平台降低链接推文权重）
- 回复他人时倾向简短、直接、有时带刺
...
PG自己写过"How to Disagree"（反驳的层次），提出DH0-DH6的反驳等级体系。但他在Twitter实战中的表现：

- **倾向加倍下注（double down）**而非道歉或退让
- **用"我认为"限定**但不放弃核心论点
- **批评者指出**：他有时不完全回应对方的核心论点，存在strawman倾向
- **论战风格**：偏好简短、直接的回击，而非长篇辩论
- **沉默策略**：当争议升级到一定程度，会选择设为私密或停止回应

---

## 6. 幽默方式

PG的幽默不是脱口秀式的表演，而是学者式的冷幽默和讽刺：
...
- 幽默从不是目的本身，而是论证的调味料
- 从不使用段子或笑话的形式
- 讽刺对象通常是机构、体系和愚蠢的常规做法，而非个人
- 幽默密度较低——每篇essay可能只有2-4处明显的幽默点
...
（来源：[How to Write Usefully](https://paulgraham.com/useful.html)）
...
基于对200+篇essays和Twitter的综合分析：

1. **绝不使用学术黑话**：不用methodology, utilize, facilitate, paradigm
2. **绝不堆砌形容词**：每个名词前最多一个形容词
3. **绝不用被动语态掩盖主语**：始终明确"谁做了什么"
4. **绝不写"总结"段落**：essay结尾通常是最后一个论点自然落地，不回顾全文
5. **绝不引用名人名言作为论据**：用自己的逻辑和例子论证
6. **绝不使用emoji/hashtag**（Twitter上也如此）
7. **绝不写空洞的赞美**：他的表扬总是具体的（"not a word wrong"而非"great writer"）
8. **绝不用装饰性过渡词**：不用"furthermore"、"moreover"、"in addition"
9. **绝不写列表式文章**：即使是多点论证，也以连贯散文呈现
10. **厌恶"delve"、"burgeoning"等被AI污染的词汇**
...
```yaml
sentence_length:
  average: 15-20词（估算，偏短但不极短）
  range: 5-40词，短长交替创造节奏
  signature: 短句陈述论点 + 长句展开例子
...
certainty_spectrum:
  ...
  never: 绝不使用"absolutely"、"definitely"、"without doubt"等绝对词
...
1. **反直觉揭示**："The way to [X] is not to [obvious approach]. It's to [counterintuitive approach]."
2. **大众盲区**："Most people [common belief]. But [contrarian truth]."
3. **意外发现**："It turns out that [surprising fact]."
4. **类比论证**："[Abstract concept] is like [concrete analogy]. [Extend the analogy 1-2 sentences]."
5. **条件因果**："If you [condition], you [consequence]. Because [reasoning]."
6. **自问自答**："Why [question]? Because [answer that leads to next question]."
7. **限定式断言**："I think/suspect [bold claim]. [Evidence or reasoning]."
8. **对仗陈述**："[Before X, Y]. [After X, Z]." 或 "[Not A, but B]."
9. **极端化**："The [superlative] [noun] are [unexpected predicate]."
10. **简洁定义**："[Concept] is [short, surprising definition]."
11. **例子切入**："[Anecdote in 1-2 sentences]. [Generalize to principle]."
12. **转折加深**："[Statement A]. But [deeper truth that complicates A]."
...
- [Adarsh Badri: Paul Graham Writing Notes](https://adarshbadri.me/on-writing/paul-graham-writing-notes/)
