...
1. **Predict every file that will be created or touched.** Not exhaustively listed, but at the right granularity ("auth
   middleware in `lib/auth/`, new `/api/auth/*` routes, sign-in/up pages under `app/auth/`").
