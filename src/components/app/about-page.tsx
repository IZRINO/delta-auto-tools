...
import { FieldUnit, StatusMatrix, DataWell } from "@/components/app/app-ui";
...
export function AboutDialog({ open, onOpenChange }: AboutDialogProps) {
  ...
  const statusItems = (() => {
    ...
    if (phase === "available") return [{ id: "update", state: "active" as const, label: `新版本 ${progress.version}` }];
    if (phase === "downloading") {
      const pct = progress.total ? Math.round((progress.downloaded / progress.total) * 100) : null;
      return [{ id: "update", state: "active" as const, label: pct != null ? `下载 ${pct}%` : `下载中 ${progress.downloaded}` }];
    }
      ...
      <DialogContent className="sm:max-w-3xl w-[min(96vw,900px)] max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid gap-2 border-2 border-[var(--chalk)] bg-[var(--carbon)] p-3">
          <div className="flex flex-wrap items-center gap-3">
            <span className="font-heading text-[clamp(1.5rem,4vw,3rem)] font-black leading-[0.85] tracking-[-0.06em] text-[var(--chalk)] uppercase">
              {bootstrap?.version ?? "—"}
            </span>
          ...
          {bootstrap?.repositoryUrl && (
            <Button variant="outline" size="sm" className="w-fit" onClick={handleOpenReleases} disabled={!isNativeShell}>
        ...
        <FieldUnit header="[ LICENSE / GPLv2+ ]" className="border-2 border-[var(--chalk)]">
          ...
          {bootstrap?.licenseUrl && (
            <div className="mt-2">
              <Button variant="outline" size="sm" onClick={async () => { try { await openUrl(bootstrap.licenseUrl); } catch { window.open(bootstrap.licenseUrl, "_blank"); } }} disabled={!isNativeShell}>
        ...
        <FieldUnit header="[ ATTRIBUTIONS ]" className="border-2 border-[var(--chalk)]">
          <div className="grid gap-px border-2 border-[var(--chalk)] bg-[var(--chalk)]">
            <div className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-x-3 bg-[var(--carbon)] px-3 py-2">
              <span className="font-mono text-[0.56rem] font-black tracking-[0.18em] text-[var(--amber)] uppercase">KIND</span>
              <span className="font-mono text-[0.56rem] font-black tracking-[0.18em] text-[var(--amber)] uppercase">NAME</span>
              <span className="font-mono text-[0.56rem] font-black tracking-[0.18em] text-[var(--amber)] uppercase">LICENSE</span>
              <span className="font-mono text-[0.56rem] font-black tracking-[0.18em] text-[var(--amber)] uppercase">URL</span>
            </div>
            {(bootstrap?.dependencies ?? DEPENDENCIES).map((dep) => (
              <div key={dep.name} className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-x-3 bg-[var(--carbon)] px-3 py-1.5 border-b border-[var(--seam)]">
                <span className={cn(
                ...
                <span className="shrink-0 font-mono text-[0.56rem] font-bold tracking-[0.08em] text-[var(--zinc)] uppercase">{dep.license}</span>
                <a
                  href={dep.url}
                  target="_blank"
