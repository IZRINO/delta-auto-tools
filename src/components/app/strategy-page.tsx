...
import {RiAddLine, RiDeleteBinLine, RiExternalLinkLine, RiRefreshLine,} from "@remixicon/react";
        ...
        <AppPage className="min-h-[calc(100dvh-4rem)] flex-1 grid-rows-[auto_minmax(0,1fr)] gap-2 overflow-hidden">
            <div
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5 bg-base-200 px-2 py-2">
                    ...
                    {activeSite ? (
                        <Button type="button" size="sm" variant="ghost" className="h-8 px-2.5"
                                onClick={handleDeleteActiveSite}>
                            <RiDeleteBinLine data-icon="inline-start"/>
                            删除
                        </Button>
                    ) : null}

                    <Button type="button" size="sm" variant={refreshPanelOpen ? "default" : "outline"}
                            className="h-8 px-2.5" onClick={handleRefreshPanelToggle} disabled={!activeSite}>
                        自动刷新：{refreshLabel}
                    </Button>

                    <Button type="button" size="sm" variant="default" className="h-8 px-2.5" onClick={handleRefresh}
                            disabled={!isNativeShell || !activeUrl}>
                        <RiRefreshLine data-icon="inline-start"/>
                        刷新
                    </Button>
                    <Button type="button" size="sm" variant="secondary" className="h-8 px-2.5"
                            onClick={handleOpenExternal} disabled={!activeUrl}>
                    ...
                    <div className="col-span-full border-t-2 border-base-content bg-base-100 p-3">
                        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.4fr)_auto]">
                            <Field className="gap-1.5">
                                ...
                                <Input id="strategy-site-short-label" value={siteForm.shortLabel} onChange={(event) => {
                                    const value = event.currentTarget.value;
                                    setSiteForm((current) => ({...current, shortLabel: value}));
                                }} placeholder="攻略站"/>
                            </Field>
                            <Field className="gap-1.5">
                                <FieldLabel htmlFor="strategy-site-label">完整名称</FieldLabel>
                                <Input id="strategy-site-label" value={siteForm.label} onChange={(event) => {
                                    const value = event.currentTarget.value;
                                    setSiteForm((current) => ({...current, label: value}));
                                }} placeholder="自定义攻略站"/>
                            </Field>
                            <Field className="gap-1.5">
                                <FieldLabel htmlFor="strategy-site-url">URL</FieldLabel>
                                <Input id="strategy-site-url" value={siteForm.url} onChange={(event) => {
                                    const value = event.currentTarget.value;
                                    setSiteForm((current) => ({...current, url: value}));
                                }} placeholder="https://example.com" onKeyDown={(event) => {
                                    if (event.key === 'Enter') {
                            ...
                            <div className="flex items-end gap-2">
                                ...
                                <Button type="button" variant="ghost" className="h-9"
                                        onClick={handleCreatePanelToggle}>收起</Button>
                            </div>
                        </div>
                        <Field className="mt-3 gap-1.5">
                            <FieldLabel htmlFor="strategy-site-description">简介（可选）</FieldLabel>
                            <Textarea id="strategy-site-description" value={siteForm.description} onChange={(event) => {
                                const value = event.currentTarget.value;
                                setSiteForm((current) => ({...current, description: value}));
                            }} placeholder="用于在站点信息中展示"/>
                        </Field>
                        {siteFormError ? <FieldDescription
                            className="mt-2 text-destructive">{siteFormError}</FieldDescription> : null}
                    </div>
                ) : null}
                ...
                {refreshPanelOpen ? (
                    <div className="col-span-full border-t-2 border-base-content bg-base-100 p-3">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Button type="button" size="sm" variant="ghost" className="h-8 px-2.5"
                                    onClick={handleRefreshPanelToggle}>收起</Button>
                        </div>
                    </div>
                ) : null}
            </div>
