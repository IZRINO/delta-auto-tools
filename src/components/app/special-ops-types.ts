...
export type LimitedSupplyAccountState = {cycleId: string | null; outcome: LimitedSupplyOutcome; checkedAtMs: number | null; matchedRegion: number | null; matchedColor: [number, number, number] | null; matchedColorIndexes?: number[]; matchedImage?: boolean; acknowledged: boolean; lastError: string | null};
...
export type ScheduledPauseSettings = {enabled: boolean; start: string; end: string};
export type MarketTaskStatus = "pending" | "running" | "completed" | "priceRecognitionFailed" | "windowClosed";
export type MarketAccountState = {day: string | null; completedCount: number; status: MarketTaskStatus; lastError: string | null; priceRetryAtMs?: number | null};
export type StationWalkthroughSnapshot = {accountIndex: number; accountTotal: number; qqAccount: string | null; exhausted: boolean};
export type SpecialOpsSettings = { enabled: boolean; paused: boolean; pausedReason?: string | null; scheduledPause?: ScheduledPauseSettings; dailyExchangeTime: string; emergencyHotkey: string; nextAccountHotkey: string; stationWalkthroughEnabled: boolean; navigationBeaconDelayMs: number; navigationSpaceDelayMs: number; navigationTabDelayMs: number; navigationSpecialOpsDelayMs: number; ammoSupplyDelayMs: number; ammoTacticalDelayMs: number; ammoExchangeEntryDelayMs: number; craftSpaceDelayMs: number; craftReopenDelayMs: number; craftConfirmPinnedDelayMs: number; wegameExecutablePath: string; gameExecutablePath: string; defaultBusinessConfig: BusinessConfig; profitFilter: ProfitFilterSettings; limitedSupply?: LimitedSupplySettings; marketPurchase?: MarketPurchaseSettings; accounts: AccountPlan[]; activeCalibrationId: string | null; calibrationEnvironments: CalibrationEnvironment[] };
...
export type LoginRunKind = "login" | "navigation" | "craft" | "ammo" | "limitedSupply" | "market" | "round" | "stationWalkthrough";
...
export type TimelineTask = {id: string; accountId: string; qqAccount: string; kind: TimelineTaskKind; stationKind: StationKind | null; ammoTargetId: string | null; note: string; scheduledAtMs: number; overdue: boolean; accountStatus: AccountStatus; profitState?: TimelineProfitState | null; mayExecuteEarlier?: boolean; manualFailure: AccountFailure | null; limitedCycleId?: string | null; limitedOutcome?: LimitedSupplyOutcome | null; marketCompletedCount?: number | null; marketTargetCount?: number | null; marketStatus?: MarketTaskStatus | null};
...
export type SpecialOpsBootstrap = { settings: SpecialOpsSettings; schedule: ScheduleSnapshot; settingsRevision: number; nowMs: number; runSnapshot: LoginRunSnapshot | null; profitRuntime: ProfitRuntimeSnapshot; stationWalkthrough?: StationWalkthroughSnapshot | null};
