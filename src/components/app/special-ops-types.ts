...
export type SpecialOpsSettings = { enabled: boolean; paused: boolean; pausedReason?: string | null; dailyExchangeTime: string; emergencyHotkey: string; navigationBeaconDelayMs: number; navigationSpaceDelayMs: number; navigationTabDelayMs: number; navigationSpecialOpsDelayMs: number; ammoSupplyDelayMs: number; ammoTacticalDelayMs: number; ammoTacticalDepartmentDelayMs: number; ammoSeasonalEntryDelayMs: number; craftSpaceDelayMs: number; craftReopenDelayMs: number; craftConfirmPinnedDelayMs: number; wegameExecutablePath: string; gameExecutablePath: string; defaultBusinessConfig: BusinessConfig; profitFilter: ProfitFilterSettings; limitedSupply?: LimitedSupplySettings; marketPurchase?: MarketPurchaseSettings; accounts: AccountPlan[]; activeCalibrationId: string | null; calibrationEnvironments: CalibrationEnvironment[] };
