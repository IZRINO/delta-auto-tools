...
import { formatTimerHotkey, counterEffectiveByGroup, moveCounterItem, counterSettingsToForm, parseCounterSettingsForm, counterRunsById, isCounterDirty } from "@/components/app/counter-utils";
...
describe("counter-utils", () => {
  ...
  it("moves counter items within the same group", () => {
    const form = counterSettingsToForm(sampleSettings());
    form.counters.push({ id: "beta", groupId: DEFAULT_COUNTER_GROUP_ID, name: "B", startValue: "1", hotkey: "F3", enabled: true });
