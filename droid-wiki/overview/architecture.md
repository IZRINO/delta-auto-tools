...
App.tsx 不使用路由库，通过 `useState<ToolId>` 切换工具页。主窗口两条壳互斥：`BlackmarkShell`（默认夜航黑标，底栏 dock）与 `ConsoleShell`（战地，左侧 Index Rail）。界面世界 `UiWorld` 写 `localStorage`，缺省 `blackmark`；与配色主题正交。透明窗口和区域选择通过 `?mode=` 查询参数分支进入以下模式，名单在 `src/lib/overlay-windows.ts`，不吃黑标：
