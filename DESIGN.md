...
界面必须像一套被拆开的作战设备控制面板：粗黑结构线、压迫感网格、巨型编号、单色告警、密集 telemetry、机械标签、扫描线与纸面噪声。它不是白色
SaaS 后台，不是柔和桌面工具，不是 shadcn 默认卡片堆叠，也不是“在旧 UI 上加一点工业感”。
...
- 主基底：深色碳基控制面 / 暗色工业终端。
- 主结构：粉笔粗线、硬格、编号、分栏、轴线。
- 数据层：军用 telemetry 小字、坐标、状态码、键位、时间、计数。
- 告警层：单一琥珀指示灯色，只用于当前选择、危险动作、运行态、关键按钮。
- 暗色唯一模式，无 light/dark 切换。
...
1. 工具导航：Morse、计时器、计数器、连发器、攻略网站、Delta 账号、游戏数据、工具箱、收藏/未开放工具。
2. Morse：区域选择、识别工作台、结果、历史、自动输入、热键录制。
3. 计时器：计时器卡片、总开关、透明窗口设置、位置校准、排序、手动触发。
4. 计数器：计数器卡片、总开关、透明窗口设置、位置校准、排序、手动触发/重置。
5. 连发器：卡片配置、触发键、目标键、间隔、抖动、补齐策略、总开关、透明窗口、排序。
...
| Role   |     Color | Usage             |
|--------|----------:|-------------------|
| Carbon | `#0C0C0B` | 主背景，深色碳基          |
| Slate  | `#171715` | 次级底板、面板、分隔        |
| Iron   | `#232320` | 卡片微升面（可选）         |
| Chalk  | `#D8D4CC` | 主文字、粗边框、结构块       |
| Zinc   | `#807C74` | 次级文字、图标、辅助边框      |
| Dust   | `#545250` | 元信息、占位、弱标签        |
| Seam   | `#2A2926` | 细网格线、内部分隔         |
| Amber  | `#E8A000` | 唯一通用强调色           |
| Rust   | `#C85400` | 仅语义警告/危险          |
| Moss   | `#3F8A30` | 仅语义成功/有效          |
| Void   | `#050504` | 小型数据井、JSON、叠加窗文本底 |

### Color Rules

- Amber 只能占画面 3%–8%。太多会变成游戏 UI 皮肤，太少没有 brutalist 冲击。
- 主按钮可以琥珀底碳字或碳底粉白字 + 琥珀色侧线。
- 普通按钮必须粉笔碳边硬线，不得使用柔和填充。
- 状态色只表达状态，不参与品牌与装饰。
...
- 直角、2px 黑边。
- Primary：琥珀底碳字，或碳底粉白字 + 琥珀色左边条。
- Secondary：碳底粉笔字，hover 反相为粉笔底碳字。
- Destructive：碳底锈字或锈色斜线标记，不用柔和 destructive 背景。
...
- 粗边框，直角。
- 内容分成编号步骤。
- QR 登录弹窗必须有明确状态机：`WAITING`, `SCANNED`, `CONFIRM`, `EXPIRED`, `FAILED`。琥珀色指示当前步骤。

### 7.8 Tables / Raw Data

- JSON 与原始响应使用 Void：极深底、粉笔字、琥珀色光标线可选。
...
- 页面主编号：`01 TIMER BOARD` 与 `02 COUNTER BOARD`。
- Timer 与 Counter 是两个独立系统，各自拥有独立页面，不再是同一页面内的 Tab 切换。
- 计时器页面显示时序配置行，显示 duration / direction / hotkey / state。
- 计数器页面显示计数配置行，显示 startValue / currentValue / hotkey / state。
- 透明窗口设置像坐标校准面板。
...
- 碳面噪声：低透明度 SVG noise 或 CSS 纹理。
- 半调点：用于大标题背景或空态，不覆盖正文。
- 斜线警戒纹：用于 locked、disabled、warning 区。
- 扫描线：仅用于 Void 数据井和透明叠加窗，不铺满主界面。
- 轻微 misregistration：琥珀色边线可偏移 1px，模拟印刷错位。
...
- Hover：反相、边框变粗、琥珀线出现。
- Active：下压 1px。
- Focus：琥珀色硬 outline，不用柔光 ring。
- Loading：文本状态码 + 进度条纹；spinner 只能辅助。
- 新增/触发反馈：一次性琥珀色 flash 或粉笔碳反相，不超过 400ms。
...
- [ ] 琥珀色是否只作为关键强调，而不是到处装饰？
- [ ] 是否看起来像暗色工业终端/军工控制板，而不是 SaaS 后台？
