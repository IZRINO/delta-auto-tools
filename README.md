...
- 已检查过的限时商品任务保留在 24 小时任务栏并显示结果：发现高价值时任务栏提供“已查看高价值商品”，确认后才出栏；未发现高价值或检查失败的“重新检查”入口在账号人工校正面板的“本周期限时商品”分区，点一次把本周期复位为待检查。自动调度每周期仍只跑一次，重复检查只由人工触发。
- 交易行购买窗口为每日 02:00–04:00；OCR 价格不高于设定值才点击购买，随后点击独立配置的最终确认点，按配置次数完成后切换账号，04:00 后本日任务关闭。交易行全局排在最后：所有账号的制作、子弹、限时商品任务跑完才开始交易行；无其他账号非交易行任务时，同账号从特勤处直接接入交易行，不重新登录。
