...
- **攻略网站工作台**：集成 `kkrb.net` 与 `orzice.com` 两类高频更新的外部攻略页面，按 Tab 切换两个站点，每个 Tab 全屏展示。该面板由 Rust 端 `strategy_fetch_page` 命令代理拉取（完整 Chrome 135 浏览器头，避开 WebView UA 引发的人机验证），前端再用 `<iframe srcDoc>` 渲染并自动注入 `<base href>`。支持按站点独立设置自动刷新间隔（30 秒 / 1 分钟 / 2 分钟 / 5 分钟 / 10 分钟 / 关闭）、立即刷新、浏览器打开、最近拉取时间显示，代理拉取失败时通过 Alert 提示并降级到外部打开。
